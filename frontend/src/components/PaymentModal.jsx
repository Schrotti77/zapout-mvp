...
export default function PaymentModal({ 
  ...
  const styles = {
    ...
    timer: { 
      fontSize: '18px', color: timeLeft < 60 ? '#dc3545' : '#666',
      marginBottom: '20px', fontWeight: timeLeft < 60 ? 'bold' : 'normal'
    },
    copyBtn: {
      background: '#f5f5f5', border: 'none', padding: '10px 20px',
      borderRadius: '8px', cursor: 'pointer', marginBottom: '15px',
      fontSize: '12px', width: '100%'
    },
    status: {
      padding: '15px', borderRadius: '12px', fontSize: '18px', fontWeight: 'bold',
      background: paymentStatus === 'paid' ? '#d4edda' : 
                  paymentStatus === 'expired' ? '#f8d7da' : '#fff3cd',
      color: paymentStatus === 'paid' ? '#155724' : 
             paymentStatus === 'expired' ? '#721c24' : '#856404'
    },
    closeBtn: {
  ...
  return (
    <div style={styles.overlay} onClick={(e) => e.target === e.currentTarget && paymentStatus !== 'paid' && onClose()}>
              ...
              return (
                <>
                  ...
                  <div style={{...styles.sats, color: '#00d4aa'}}>
                    {liveSats === null ? '🔄 Live-Umrechnung...' : `⚡ ${displaySats.toLocaleString()} sats`}
                  </div>
                </>
              );
            })()}
