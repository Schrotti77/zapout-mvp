...
export default function CartDrawer({ isOpen, onClose, setScreen }) {
  ...
  const styles = {
    overlay: {
      ...
      display: isOpen ? 'flex' : 'none', alignItems: 'flex-end'
    },
    drawer: {
    ...
    delBtn: { background: '#fee', color: '#c00', border: 'none', padding: '8px', borderRadius: '8px', cursor: 'pointer' },
    total: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '20px', paddingTop: '20px', borderTop: '2px solid #eee' },
    totalLabel: { fontSize: '18px', fontWeight: 'bold' },
    totalValue: { fontSize: '24px', fontWeight: 'bold', color: '#f7931a' },
    checkoutBtn: { width: '100%', background: '#f7931a', color: '#fff', border: 'none', padding: '16px', borderRadius: '12px', fontSize: '16px', fontWeight: 'bold', cursor: 'pointer', marginTop: '16px' },
    clearBtn: { background: '#666', color: '#fff', border: 'none', padding: '10px', borderRadius: '8px', cursor: 'pointer', marginTop: '10px', width: '100%' },
          ...
          <>
            {items.map(item => (
              <div key={item.id} style={styles.item}>
                <div style={styles.itemInfo}>
                  ...
                  <p style={styles.itemPrice}>€{(item.price_cents / 100).toFixed(2)} × {item.quantity}</p>
                </div>
                <span style={styles.itemTotal}>€{((item.price_cents * item.quantity) / 100).toFixed(2)}</span>
                <button style={styles.delBtn} onClick={() => removeItem(item.id)}>🗑️</button>
              </div>
            ))}
            ...
            {checkoutResult && (
              <div style={{...styles.drawer, marginTop: '20px', background: '#f0fff4'}}>
                ...
                <p><strong>Payment ID:</strong> #{checkoutResult.payment_id}</p>
                <p><strong>Betrag:</strong> €{(checkoutResult.amount_cents / 100).toFixed(2)} ({checkoutResult.amount_sats} sats)</p>
                  ...
                  <div style={{margin: '10px 0'}}>
                    ...
                    <ul style={{margin: '5px 0'}}>
                      {checkoutResult.items.map((item, i) => (
                        <li key={i} style={{fontSize: '14px'}}>{item.name} × {item.quantity || item.qty}</li>
                      ))}
                    </ul>
                  </div>
                )}

                <div style={{background: '#fff', padding: '12px', borderRadius: '8px', marginTop: '10px'}}>
                  <p style={{margin: '0 0 5px 0', fontSize: '12px', color: '#666'}}>Zahle jetzt mit Lightning:</p>
                  <p style={{margin: 0, fontFamily: 'monospace', fontSize: '11px', wordBreak: 'break-all'}}>
                    {checkoutResult.invoice?.bolt11 || checkoutResult.invoice?.message || 'Invoice wird erstellt...'}
                  </p>
                </div>

                <button style={{...styles.checkoutBtn, background: '#22c55e', marginTop: '12px'}} onClick={closeResult}>
                  ✓ Fertig
                </button>
              </div>
            )}
