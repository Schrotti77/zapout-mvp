    ...
    try {
      if (swapType === 'lightning-to-cashu') {
        ...
        if (data.payment_request) {
          ...
          const balRes = await fetch(API_URL + '/cashu/balance', { headers: { Authorization: 'Bearer ' + token } });
    ...
    return (
      <div style={{ padding: '20px', paddingBottom: '100px' }}>
        <div style={{ textAlign: 'center', marginBottom: '24px' }}>
          <div style={{ fontSize: '48px', marginBottom: '16px' }}>{result.status === 'pending' ? '⏳' : '✅'}</div>
          <p style={{ fontSize: '20px', fontWeight: 'bold', color: '#ffffff' }}>
            {result.status === 'pending' ? 'Rechnung erstellt!' : 'Bereit!'}
          </p>
          <p style={{ fontSize: '28px', fontWeight: 'bold', color: '#f7931a', marginTop: '8px' }}>{result.amount} Sats</p>
        </div>

        <div style={cardStyle}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
            <span style={{ color: '#666666', fontSize: '14px' }}>Richtung:</span>
            <span style={{ color: '#ffffff', fontSize: '14px' }}>{result.type === 'lightning-to-cashu' ? '⚡ → 🪙' : '🪙 → ⚡'}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
          ...
          {result.invoice && (
            <div style={{ marginTop: '16px', paddingTop: '16px', borderTop: '1px solid #222222' }}>
              <p style={{ color: '#666666', fontSize: '12px', marginBottom: '8px' }}>Lightning Rechnung:</p>
          ...
          {result.instruction && (
            <p style={{ color: '#eab308', fontSize: '12px', marginTop: '12px' }}>💡 {result.instruction}</p>
          )}
        </div>
    ...
    <div style={{ padding: '20px', paddingBottom: '100px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '20px' }}>
        <div style={cardStyle}>
          ...
          <p style={{ fontSize: '24px', fontWeight: 'bold', color: '#ffffff', marginTop: '4px' }}>{lightningBal} <span style={{ fontSize: '14px', color: '#666666' }}>sats</span></p>
        </div>
        <div style={cardStyle}>
          <p style={{ color: '#666666', fontSize: '12px' }}>🪙 Cashu</p>
          <p style={{ fontSize: '24px', fontWeight: 'bold', color: '#f7931a', marginTop: '4px' }}>{cashuBal} <span style={{ fontSize: '14px', color: '#666666' }}>sats</span></p>
        </div>
      </div>

      {/* Swap Type Selection */}
      <h3 style={{ fontSize: '14px', fontWeight: '600', color: '#ffffff', marginBottom: '8px' }}>Swap Typ</h3>
      <p style={{ color: '#666666', fontSize: '14px', marginBottom: '16px' }}>Wähle die Richtung</p>
      
      <div style={{ marginBottom: '20px' }}>
        <button
          onClick={() => { setSwapType('lightning-to-cashu'); setError(null); }}
          style={{
            ...cardStyle,
            marginBottom: '12px',
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '12px', color: '#666666' }}>Sats einzahlen & als Cashu erhalten</div>
            </div>
          </div>
        </button>
        
        <button
          onClick={() => { setSwapType('cashu-to-lightning'); setError(null); }}
          style={{
            ...cardStyle,
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '12px', color: '#666666' }}>Cashu verbrennen & Lightning erhalten</div>
            </div>
          </div>
        </button>
      </div>
      ...
      {/* Amount Input */}
      <h3 style={{ fontSize: '14px', fontWeight: '600', color: '#ffffff', marginBottom: '8px' }}>Betrag</h3>
      ...
      >
        {loading ? '⏳ Wird getauscht...' : swapType === 'lightning-to-cashu' ? '📥 Sats einzahlen' : '📤 Sats auszahlen'}
      </button>

      {/* Info Box */}
      <div style={{ ...cardStyle, marginTop: '20px', padding: '16px' }}>
        <p style={{ fontSize: '12px', color: '#666666' }}>
          💡 <strong style={{ color: '#999' }}>Lightning → Cashu:</strong> Bezahle eine Lightning Rechnung und erhalte Cashu Tokens
        </p>
        <p style={{ fontSize: '12px', color: '#666666', marginTop: '8px' }}>
          💡 <strong style={{ color: '#999' }}>Cashu → Lightning:</strong> Verbrenne Cashu Tokens und erhalte eine Lightning Auszahlung
        </p>
      </div>
    </div>
  );
